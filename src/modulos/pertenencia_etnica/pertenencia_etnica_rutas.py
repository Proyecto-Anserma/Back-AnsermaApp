...
from .pertenencia_etnica_modelos import *
from .pertenencia_etnica_servicio import *

router = APIRouter()

@router.get("/obtener_todos/", response_model=List[PertenenciaEtnicaResponse])
async def obtener_todos(db: AsyncSession = Depends(get_db_anserma)):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error al obtener todas las pertenencias étnicas: {str(e)}"
        )